...
function loadNavbar(selector = '#navbar') {
    fetch('../components/navbar.html')
        ...
        .then((data) => {
            ...
            else if (window.location.href.includes('index.html')) localStorage.setItem('currentUser', null);
        })
...
function getQuesNumberClassName(i) {
    ...
    } else if (currentUser?.questions[i]?.yourAnswer === '' && currentUser?.questions[i]?.timeTaken > 0 && currentUser?.questions[i]?.timeTaken < 60) {
...
function getPregressWidth() {
    const filterAns = currentUser?.questions.filter((item) => item.yourAnswer !== '' || (item.yourAnswer === '' && item.timeTaken > 0));
...
function loadQuizReview() {
    ...
    document.getElementById('totalQuestions').textContent = ` ${currentUser.questions.length} Questions`;
    ...
    document.getElementById('answered').textContent = `${currentUser.score}/${currentUser.questions.length}`;
    ...
    document.getElementById('time-taken').textContent = formatTime(currentUser.questions.length * 60 - currentUser.totalTimeTaken);
    ...
    currentUser.questions.forEach((ques) => {
        ...
        td4.className = 'w-[15%]';
        td4.innerHTML = ques.correct_answer === ques.yourAnswer ? `<i class="fa-solid fa-check"></i>` : `<i class="fa-solid fa-xmark"></i>`;
...
function startTimer() {
    ...
    timerValue = currentUser.questions[currentQuestion.id].timeTaken > 0 ? 60 - currentUser.questions[currentQuestion.id].timeTaken : 60;
...
function remaningTimer() {
    ...
    timeLeft = currentUser.totalTimeTaken === 0 ? currentUser.questions.length * 60 : currentUser.totalTimeTaken;
...
const setQuizUI = (ques) => {
    ...
    if (document.getElementById('user-detail')) document.getElementById('user-detail').style.display = 'none';
    if (document.getElementById('quiz-questions')) document.getElementById('quiz-questions').style.display = 'flex';
...
async function startQuiz() {
    ...
    if (name && email) {
        ...
        document.getElementById('enter-name').style.display = name == '' ? 'block' : 'none';
        document.getElementById('enter-email').style.display = email == '' ? 'block' : 'none';
    }
}
...
const saveNext = (index) => {
    ...
    const queNum = document.getElementById(`question-number-${currentQuestion.id + 1}`);
    ...
    stopTimer();
    const nextQues = index !== undefined ? currentUser.questions[index] : currentUser.questions[currentQuestion.id + 1];
    if (nextQues) {
        const options = [...nextQues.incorrect_answers, nextQues.correct_answer].sort(() => Math.random() - 0.5);
...
function submitQuiz() {
    ...
    currentUser.questions.forEach((item) => {
        if (item.yourAnswer === item.correct_answer) {
            ...
            skipped++;
        } else if (item.yourAnswer !== '' && item.yourAnswer !== item.correct_answer) {
            incorrect++;
...
function checkQuizStatus() {
    ...
    if (cUser?.questions.length > 0) {
        currentUser = cUser;
        const index = currentUser.questions.findIndex((item) => item.timeTaken < 60 && item.yourAnswer === '');
...
async function logAllResultsLength() {
    ...
    for (const [key, path] of Object.entries(urls)) {
        try {
            ...
            const data = await res.json();
            const len = Array.isArray(data.results) ? data.results.length : 0;
            console.log(`${key}: results.length = ${len}`);
        } catch (e) {
            console.error(`Error reading ${key} (${path}):`, e);
        }
    }
}
